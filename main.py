...
def shooting(player):
    if sable1.check_hit(player.x, player.y, 0):
        player.score = sable1.apply_effect(player.score)

    if sable2.check_hit(player.x, player.y, 1):
        player.score = sable2.apply_effect(player.score)

    if sable3.check_hit(player.x, player.y, 2):
        player.score = sable3.apply_effect(player.score)

    if bomb1.check_hit(player.x, player.y, 3):
        player.score = bomb1.apply_effect(player.score)

    if speed.check_hit(player.x, player.y, 4):
        player.speed = speed.apply_effect(player.speed)

    if time1.check_hit(player.x, player.y, 5):
        player.time = time1.apply_effect(player.time)

    if bullet1.check_hit(player.x, player.y, 6):
...
def main():
    ...
    while running:
        ...
        display_time(screen, player1.time, player2.time)
        display_score(screen, player1, player2)
